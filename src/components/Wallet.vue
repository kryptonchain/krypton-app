  ...
  <ion-app>
    <ion-header>
      <ion-toolbar color="dark">
        <ion-title>
          <img alt="logo" height="40" src="../assets/krypton.png" /> Krypton
          Wallet | Wallet
        </ion-title>
      </ion-toolbar>
    </ion-header>
    <ion-content padding>
      ...
      <ion-button color="dark" @click="createWallet" full
        >Generate Wallet</ion-button
      >
      <div>Your wallet address: {{ getPrimaryAddress }}</div>
      <div>
        <ion-icon name="sync"></ion-icon> Sync status {{ getSyncStatus }}
      </div>
    ...
    createWallet() {
      ...
      this.wallet.on(
        ...
        (walletBlockCount, localDaemonBlockCount, networkBlockCount) => {
          mutations.setSyncStatus(
            `New sync status: ${walletBlockCount} / ${localDaemonBlockCount}`
          );
          console.log(
            `New sync status: ${walletBlockCount} / ${localDaemonBlockCount}`
          );
